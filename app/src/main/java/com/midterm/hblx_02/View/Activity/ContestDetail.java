...
public class ContestDetail extends AppCompatActivity implements View.OnClickListener{
    ...
    private com.midterm.hblx_02.ViewModel.myResource myResource;
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        setContentView(R.layout.activity_contest_detaill);
    ...
    private void createListViewAnswer(final int index) throws Exception{
        ...
        for (int i=0;i<question.getAnswer().size();i++) {
            ...
            if (isShowRusult()) {
                ...
                for (Integer temp : question.getUserResult()) {
                    if (temp == i) {
                        answerTrue = true;
                        break;
                    }
        ...
        if (question.getUserResult() != null){
            for(Integer i: question.getUserResult()){
...
//                        btn.setBackgroundResource(R.color.white);
                    ...
                    question.setUserResult(new ArrayList<Integer>());
                    for (int i=0;i<question.getAnswer().size();i++){
                        row = (Custom_Row_Answer) parent.getItemAtPosition(i);
                        if (row.isBit()){
                            question.getUserResult().add(i);
                        }
                    }
                }
            }
    ...
    private void SubmitContest(){
        ...
        for (int i=0;i<20;i++){
            ...
            if (myCompare(question.getUserResult(),question.getResult())) count++;
        }
        if(count>=18){
        ...
        btnTime.setBackgroundResource(R.color.colorPrimary3);
    ...
    private void btnSubmit_Click(){
        ...
        if (!isShowRusult()){
            ...
            for(int i=0;i<20;i++){
                ...
                if (question.getUserResult().size()==0){
                    flag = true;
                    break;
                }
            }
